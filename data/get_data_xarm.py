...
class qt_cy(QMainWindow):
    ...
    def xarm_init(self):
        self.arm = XArmAPI("192.168.1.200", is_radian=True)
    ...
    def qt_init(self):
        ...
        fx, fy = self.intr2.fx, self.intr2.fy
        ppx, ppy = self.intr2.ppx, self.intr2.ppy
        ...
        # 加载UI文件
        self.img_num = 0
        self.ui = QtUiTools.QUiLoader().load(f"model/calib.ui")  # 加载文件
        ...
        # 机械臂控制
        move_buttons = {
            3: 'move_x+', 4: 'move_x-', 5: 'move_y+', 6: 'move_y-',
            7: 'move_z+', 8: 'move_z-', 9: 'move_rx+', 10: 'move_rx-',
            11: 'move_ry+', 12: 'move_ry-', 13: 'move_rz+', 14: 'move_rz-'}
        # 为按钮按下事件和释放事件连接相应的函数
        for button_id, msg in move_buttons.items():
            push_button = getattr(self.ui, f'pushButton_{button_id}')
            # 按下按钮时发送相应的消息
            push_button.pressed.connect(lambda msg=msg: self.button_multi(msg=msg))
            # 释放按钮时发送 'stop' 消息
            push_button.released.connect(lambda: self.button_multi(msg='stop'))
        # 终端显示
        sys.stdout = self
        sys.stderr = self
    ...
    def multi_th(self, msg):
        move_map = {"move_x+": (1000, 'x'),"move_x-": (-1000, 'x'),"move_y+": (1000, 'y'),"move_y-": (-1000, 'y'),
            "move_z+": (1000, 'z'),"move_z-": (-1000, 'z'),"move_rx+": (2.5, 'roll'),"move_rx-": (-2.5, 'roll'),
            "move_ry+": (2.5, 'pitch'),"move_ry-": (-2.5, 'pitch'),"move_rz+": (2.5, 'yaw'),"move_rz-": (-2.5, 'yaw')}
        # 检查msg是否在字典中并执行对应的动作
        if msg in move_map:
            v_pos, a_pos = 200, 100
            self.clear_xarm()
            value, axis = move_map[msg]
            position = {axis: value}  # 动态构造位置参数
            self.arm.set_position(**position, speed=v_pos, mvacc=a_pos, wait=True)
            _, pos = self.arm.get_position(is_radian=True)
            print(pos)
        elif msg == "open_cam":
            ...
            _, pos = self.arm.get_position(is_radian=True)
            img_adress = f"{self.folder_adress}/{self.img_num}.png"
            ret = cv2.imwrite(img_adress, self.color_image2)
